...
from .calculation import (InteractionEntropyCalc, C2EntropyCalc,
                          IEout, C2out,
                          emapping, flatten, get_std,
                          mask2list)
...
class MMPBSA_API():
    ...
    def setup_file(self, fname: Union[Path, str]):
        ...
        if not self.fname.exists():
            raise FileExistsError(f"cannot find {self.fname}!")
    ...
    def load_file(self, fname: Union[Path, str] = None):
        ...
        if self.fname.suffix == '.xbfree':
            self._get_fromBinary(self.fname)
        elif self.fname.suffix == '.mmxsa':
            self._get_fromOldBinary(self.fname)
        else:
            raise TypeError('Unknown file')
    ...
    def _get_fromBinary(self, ifile):
        with open(ifile, 'rb') as bf:
            ...
            _info = pickle.load(bf)
            info = pickle.load(bf)
            self.app_namespace = self._get_namespace(info, 'xbfree')
    ...
    def _get_fromOldBinary(self, ifile):
        with open(ifile, 'rb') as bf:
            info = pickle.load(bf)
            self.app_namespace = self._get_namespace(info, 'mmxsa')
            bdata = pickle.load(bf)
            self._oringin = {'normal': bdata.normal, 'mutant': bdata.mutant, 'decomp_normal': bdata.decomp_normal,
                             'decomp_mutant': bdata.decomp_mutant, 'mutant-normal': bdata.mut_norm}
            self._finalize_reading(ifile)
    ...
    def _get_namespace(app, tfile):
        ...
        if tfile == 'xbfree':
            INFO = app.INFO
            # FIXME: create a temp file??? Identify the method used
            # write the pdb file
            with open(app.FILES.complex_fixed, 'w') as opdb:
                opdb.write(app.INFO['COM_PDB'])
        else:
            # FIXME: create a temp file??? Identify the method used
            # write the pdb file
            with open(app.FILES.complex_fixed, 'w') as opdb:
