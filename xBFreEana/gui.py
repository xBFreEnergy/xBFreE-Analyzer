...
from xBFreEana import __version__
...
class GMX_MMPBSA_ANA(QMainWindow):
    ...
    def _initialize_systems(self):
        ...
        if (
                ...
                self.systems[1]['namespace'].INPUT['rism']['rismrun'] or
                self.systems[1]['namespace'].INPUT['gbnsr6']['gbnsr6run']
        ):
        ...
        if self.optionWidget_c.currentIndex() == 0:  # Chart Options
            ...
            for x in sys_list:
                energyrun = (self.systems[x]['namespace'].INPUT['gb']['gbrun'] or
                             ...
                             self.systems[x]['namespace'].INPUT['rism']['rismrun'] or
                             self.systems[x]['namespace'].INPUT['gbnsr6']['gbnsr6run']
                             )
            ...
            if iq.qsize():
                replot_energy = True
                from dialogs import ProcessingProgressBar
                    ...
                    if c == 1:
                        ...
                        col_values.append(
                            ki2energy(float(self.corr_sys_sel_w.item(r, c).text()),
                                      self.systems[int(self.corr_sys_sel_w.item(r, 0).text())]['namespace'].INPUT['general'][
                                          'temperature']))
                if col_values:
    ...
    def process_data(self, results: Queue):
        ...
        for i, _ in enumerate(range(size), start=1):
            ...
            if not self.all_systems_active:
                continue
            if not frange_base:
                frange_base = [api.app_namespace.INPUT['general']['startframe'], api.app_namespace.INPUT['general']['endframe'],
                               api.app_namespace.INPUT['general']['interval'], api.app_namespace.INPUT['nmode'][
                                   'nmstartframe'],
                               api.app_namespace.INPUT['nmode']['nmendframe'], api.app_namespace.INPUT['nmode'][
                                   'nminterval']]
                continue
            if frange_base != [api.app_namespace.INPUT['general']['startframe'], api.app_namespace.INPUT['general']['endframe'],
                               api.app_namespace.INPUT['general']['interval'], api.app_namespace.INPUT['nmode'][
                                   'nmstartframe'],
                               api.app_namespace.INPUT['nmode']['nmendframe'], api.app_namespace.INPUT['nmode'][
                                   'nminterval']]:
